                ...
                <div class="card shadow p-3 bg-body-tertiary rounded">
                  <div class="card-body">
                    ...
                    <div class="table-responsive">
                      <table class="table table-hover">
                        ...
                        <tbody>
                          ...
                          if (!empty($searchKeyword)) {
                            $searchQuery = "WHERE activityName LIKE '%$searchKeyword%' OR activityType LIKE '%$searchKeyword%' OR activityStart LIKE '%$searchKeyword%' OR activityEnd LIKE '%$searchKeyword%' OR activityStatus LIKE '%$searchKeyword%' OR volunteerID LIKE '%$searchKeyword%' ";
                          }

                          // Query to fetch limited records based on the search query
                          $countQuery = "SELECT COUNT(*) AS total FROM proposedactivity $searchQuery";
                          ...
                          $query = "SELECT * FROM proposedactivity $searchQuery LIMIT $start, $limit";
                      ...
                      <td>{$row['activityEnd']}</td>
                      <td>{$row['activityStatus']}</td>
                      <td>{$row['volunteerID']}</td>
                      <td>
                      <a type='button' class='btn btn-gradient-dark btn-icon-text' href='activityEdit.php?activityID={$row['activityID']}'> Edit <i class='mdi mdi-file-check btn-icon-append'></i></a>
                      <a type='button' class='btn btn-gradient-danger btn-icon-text' onclick='return confirm(\"Are you sure you want to delete this activity?\")' href='activityDelete.php?activityID={$row['activityID']}'> Delete <i class='mdi mdi-delete'></i></a>
                      </td>
                      </tr>
                      ";
                          }
